...
public class Ship {
	...
	int[] xcoors = new int[3];
	int[] ycoors = new int[3];
	int speed = 10;
	
	int totalPoints;
	Planet target;
	
	boolean reachedPlanet = false;
	...
	int ColorR = r.nextInt(255);
	int ColorG = r.nextInt(255);
	int ColorB = r.nextInt(255);
	...
	public void setStatusLabel(Planet planet) {
		...
		lblStatus = new JLabel("Ship " + shipName + ": " + planet.nameOfPlanet + " Arrived: " + checkReachedPlanet());
	...
	public void setReachedPlanet() {
		if((xcoors[1] > (target.pX - target.sizeDiameter/2) && xcoors[1] < (target.pX + target.sizeDiameter/2)) && (ycoors[1] > (target.pY - target.sizeDiameter/2) && ycoors[1] < (target.pY + target.sizeDiameter/2))) {
			reachedPlanet = true;
		}
	}
	...
	public void setLandedCoordinates(int xcoor, int ycoor) {
		xcoors[0] = xcoor - 10;
		xcoors[2] = xcoor + 5;
